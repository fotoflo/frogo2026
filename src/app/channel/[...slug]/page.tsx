...
export async function generateMetadata({
  ...
  const description =
    channel.description || `${channel.name} channel on Frogo.tv`;
...
async function getChannel(segments: string[]) {
  ...
  return { channel, videos: (videos ?? []) as ChannelVideo[], allChannels };
}
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
      ...
      <div className="space-y-3">
        {videos.map((video: ChannelVideo, i: number) => (
          <Link
            key={video.id}
