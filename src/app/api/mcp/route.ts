 ...
 *   update_channel      — edit an existing channel (name/slug/description/icon/parent)
 *   delete_channel      — delete a channel (requires force=true if it has children)
...
async function toolUpdateChannel(
  service: Service,
  auth: ResolvedToken,
  args: {
    id?: string;
...
async function toolDeleteChannel(
  service: Service,
  auth: ResolvedToken,
...
async function callTool(
  ...
  switch (name) {
    ...
    case "delete_channel":
      return toolDeleteChannel(
        service,
        auth,
        args as { id?: string; force?: boolean }
      );
    case "add_video":
      return toolAddVideo(
        service,
